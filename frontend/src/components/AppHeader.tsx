    ...
    <header className="flex items-center justify-between gap-4 pt-2">
      <div className="min-w-0">
        <div className="mb-4 flex items-center gap-2.5">
          <div className="flex h-8 w-8 items-center justify-center rounded-md border border-white/20 bg-white text-xs font-black text-black">SMC</div>
          <div>
      ...
      <div className="h-12 w-12 shrink-0 overflow-hidden rounded-lg border border-white/10 bg-white/[0.06] p-0.5">
        {photoUrl ? <img src={photoUrl} alt="Telegram profile" className="h-full w-full rounded-[6px] object-cover" /> : <div className="flex h-full w-full items-center justify-center rounded-[6px] bg-white/10 text-sm font-semibold text-zinc-200">{firstName?.[0]?.toUpperCase() ?? 'S'}</div>}
      </div>
    </header>
  );
}
